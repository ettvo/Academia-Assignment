...
# make connected first

def addEdge(G: nx.Graph, x: int):
    for i in range(x):
        G.add_edge(i, x - i if i != x - i else x - i + 1, weight=(rand.randint(750, 1000)))
        G.add_edge(i, round((x - i)/2) if i != round((x - i)/2) else round((x - i)/2) - 10, weight=(rand.randint(800, 1000)))
        G.add_edge(i, round(x/2 + i) if (round(x/2 + i) != i and round(x/2 + i) < x) else round(i/2), weight=(rand.randint(900, 1000)))
        G.add_edge(i, i + 2 if i + 2 < x else i - 3, weight=(rand.randint(850, 1000)))
